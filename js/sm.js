			...
			d.forEach(function(card)
			{
				...
				{					
					...
					if(typeof(info) !== "undefined")
					{
						...
						s = parseInt(s)+parseInt(dist.num_cards);
					}
				});
				cardList[i].bcx = b;
	...
	cardList.forEach(function(card)
	{
		...
		card.dist.forEach(function(info)
		{
			...
			$("#tabl").append("<tr class='id"+card.id+" w3-hide'><td>"
				+info.edition+"</td><td>"
				+new Intl.NumberFormat().format(info.price)+" $/BCX</td><td>"+new Intl.NumberFormat().format((info.bcx*info.price).toFixed(0))+" $</td><td>"
